...
and the animation clock can fail independently: in the SVG section's preview
...
scroll-color-sweep's GSAP tab once passed with its colour never moving. Setup
is not motion. Where no single property qualifies, measure the never-ran
baseline rather than guess it: run the same page with the animation's JS
removed, and require the real run to beat it.
